...
class detailSwitch extends Component {
  ...
  onRight = () => {
    ...
    showInt += 1;
    if (showInt > this.dataArray.length - 1) {
      showInt = this.dataArray.length - 1;
    }
    this.setState({ showInt, imgAnim });
    this.bannerImg.next();
  ...
  render() {
    const imgChildren = this.dataArray.map((item, i) => (
    ...
    const textChildren = this.dataArray.map((item, i) => {
    ...
    return (<div
      ...
      style={{ background: this.dataArray[this.state.showInt].background }}
    >
      <div className={this.props.className}>
        ...
        <TweenOneGroup enter={{ opacity: 0, type: 'from' }} leave={{ opacity: 0 }}>
          ...
          {this.state.showInt < this.dataArray.length - 1 && <Icon type="right" key="right" onClick={this.onRight} />}
        </TweenOneGroup>
      </div>
    </div>);
  }
